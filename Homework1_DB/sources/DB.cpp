...
DB::~DB() {
    for (size_t i = 0; i < GeneralData_.size(); ++i) { delete GeneralData_[i]; }

    GeneralData_.clear();
}

void DB::SaveFile(std::string Name) {
    std::ofstream output((Name == "\0" ? DBName_ : Name) + ".db");
    output << ToString();
    output.close();
}
...
std::vector<BasicStorage *> DB::GroupByPercentage(const size_t Percentage) {
    ...
    for (auto Node : GeneralData_) {
        if (Node->GetPercentage() < Percentage) {
